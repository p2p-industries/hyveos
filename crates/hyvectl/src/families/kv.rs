        ...
        match self {
            Kv::Put { key, value, topic } => {
                boxed_try_stream! {
                    ...
                    yield CommandOutput::new_result("KV Put")
            ...
            Kv::Get { key, topic } => {
                boxed_try_stream! {
                    ...
                    match result {
                        Some(res) => yield CommandOutput::new_result("KV Get")
                        ...
                        None => yield CommandOutput::new_result("KV Get")
                            .with_field("key", OutputField::String(key))
                            .with_field("topic", OutputField::String(topic))
            ...
            Kv::Provide { key, topic } => {
                boxed_try_stream! {
                    ...
                    yield CommandOutput::new_result("KV Provide")
                        .with_field("key", OutputField::String(key))
                        .with_field("topic", OutputField::String(topic))
            ...
            Kv::GetProviders { key, topic } => {
                boxed_try_stream! {
                    ...
                    while let Some(event) = providers_stream.next().await {
                        ...
                        match event {
                            Ok(provider) => yield CommandOutput::new_result("KV GetProviders")
                                .with_field("key", OutputField::String(key.clone()))
                            ...
                            Err(e) => yield CommandOutput::new_error("KV GetProviders", &e.to_string())
                        }
                    }
                }
            },
