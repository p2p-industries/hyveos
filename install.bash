...
if [ "$EUID" -ne 0 ]; then
	echo "Not root, escalating permissions"
fi
...
if [ -x "$(command -v docker)" ]; then
	...
	echo -e "${YELLOW}2. Install Docker using docker conveniance script. (easy + potentially unsafe)${RESET}"
...
echo -e "${RED}WARNING: If you choose yes in the following step the wifi interface over which you might be connected will change it's config and you will now longer be able to connect to it conventionally. Please make sure you are connected via an alternative interface or ok with loosing access. You can still access the computer with attached monitor and keyboard or via other devices in the network.${RESET}"
